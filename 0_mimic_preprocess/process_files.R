...
import_mimic <- function(path, out_dir_path, verbose = interactive()) {
    ...
    feature_codes <- fread(file.path("/app/features", "feature_codes.csv"))
    print(feature_codes)    
    d_labitems <- fread(file.path(extdata_path, "d_labitems.csv"))
    print(names(d_labitems))
    print(names(feature_codes))
    ...
    transfers <- fread(file.path(path, "hosp", "transfers.csv"))
    ...
    patients <- fread(
        file.path(path, "hosp", "patients.csv"),
        drop = c("dod")
    )
    ...
    labevents <- merge(
        labevents, patients[, .(subject_id, Sex, Birth, anchor_year_group)], by = "subject_id",
    ...
    labevents[,
        c("subject_id", "hadm_id", "anchor_year_group",
            ...
            "Sender", "Episode", "Time", "TargetIcu", "SecToIcu", name_mapping$label
        ),
        with = FALSE
    ]
