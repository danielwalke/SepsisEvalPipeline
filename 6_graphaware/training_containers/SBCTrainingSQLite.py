...
class SBCTraining:
    ...
    def get_node_split_containers(self, connector):
        ...
        # Removed the Cypher 'n.' prefix so it maps directly to SQLite columns.
        self.train_split_info = NodeSplitInformation(
            train_seed_ids, self.train_label_name, "WHERE id IN $ids", "SBC_TRAIN"
        )
        self.val_split_info = NodeSplitInformation(
            val_seed_ids, self.val_label_name, "WHERE id IN $ids", "SBC_VAL"
        )
        self.test_split_info = NodeSplitInformation(
            connector.get_ids('SBC_TEST'), self.test_label_name, "", "SBC_TEST"
        )
        self.ext_test_split_info = NodeSplitInformation(
            connector.get_ids('SBC_EXT_TEST'), self.ext_test_label_name, "", "SBC_EXT_TEST"
        )
        
        return NodeSplitContainer(
            self.train_split_info, 
            self.val_split_info, 
            self.test_split_info, 
            self.ext_test_split_info
        )
